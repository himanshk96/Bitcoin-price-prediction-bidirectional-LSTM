...
def load_data(filename, sequence_length):
    ...
    """
    #Read the data file
    raw_data = pd.read_csv(filename, dtype = float, encoding='utf-8').values
...
def fit_model(model, X_train, Y_train, batch_num, num_epoch, val_split):
    ...
    start = time.time()
    X_train=X_train[0:544]
    Y_train=Y_train[0:272]
    Y_train=Y_train.reshape(544,-1)
...
X_train, Y_train, X_test, Y_test, Y_daybefore, unnormalized_bases, window_size = load_data("bitcoinprices.csv", 50)
print (X_train.shape)
print (Y_train.shape)
print (X_test.shape)
print (Y_test.shape)
...
model, training_time = fit_model(model, X_train, Y_train, 1024, 100, 0.05)
